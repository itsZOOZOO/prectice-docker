...
async function loadChart(id: number) {
  ...
  try {
    const [c, notes, bills, receipts, rxs, appts, tasks] = await Promise.all([
      ...
      api<{ note_id: number, body: string, created_at: string }[]>(`/clients/${id}/notes`),
    ...
    for (const n of notes) {
      items.push({ id: `note-${n.note_id}`, kind: 'note', title: 'Note', body: n.body, at: n.created_at })
    }
    ...
    for (const a of appts.items) {
      items.push({
        ...
        kind: 'appointment',
        title: `Appt · ${a.appointment_time} · ${a.status}`,
...
<template>
  <div class="flex h-full min-h-0 w-full flex-row overflow-hidden">
    <!-- List -->
    <div class="flex h-full w-[320px] shrink-0 flex-col border-r border-slate-200 bg-white">
    ...
    <!-- Chart -->
    <div class="flex h-full min-h-0 min-w-0 flex-1 flex-col bg-[#F0F4F8]">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          ...
          <ul v-else class="space-y-3">
            <li
              ...
              :key="item.id"
              class="rounded-xl border border-slate-200 border-l-4 bg-white px-4 py-3 shadow-sm"
              :class="kindColor(item.kind)"
            >
              <div class="flex items-center justify-between gap-2">
                <p class="text-sm font-medium text-[#1C2B35]">{{ item.title }}</p>
                <p class="shrink-0 text-xs text-slate-400">{{ new Date(item.at).toLocaleString() }}</p>
              </div>
              <p v-if="item.body" class="mt-1 whitespace-pre-wrap text-sm text-slate-600">{{ item.body }}</p>
            </li>
