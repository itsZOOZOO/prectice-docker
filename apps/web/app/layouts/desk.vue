...
const { view, title, setView, openPatient } = useDeskUrl()
const { api } = useApi()
...
provide('deskOpenBook', () => { bookOpen.value = true })
...
<template>
  <div class="flex h-svh flex-row overflow-hidden bg-[#F0F4F8] text-[#1C2B35]">
    <aside
      class="flex h-full shrink-0 flex-col border-r border-slate-200 bg-white transition-[width] duration-200"
    ...
    >
      ...
      <nav class="flex-1 space-y-1" :class="collapsed ? 'p-2' : 'p-3'">
        <button
          v-for="item in nav"
          :key="item.key"
          type="button"
    ...
    <DeskBookModal v-model:open="bookOpen" @booked="refreshBadges" />
  </div>
</template>
